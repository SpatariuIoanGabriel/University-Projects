...
WHERE Name LIKE 'J%' And Name NOT LIKE '%Doe';  
...
INNER JOIN Managers ON Shops.Sid = Managers.Mid
...
RIGHT JOIN Managers ON Shops.Sid = Managers.Mid;
...
SELECT *
FROM VideoGamesCompany
WHERE Vid IN (SELECT Mid FROM Managers WHERE Mid < 5);

-- EXISTS --
SELECT *
FROM Shops
WHERE EXISTS (SELECT * FROM Managers WHERE Mid < 5 AND Mid = Sid)
...
FROM (SELECT G.Gid, G.Name, G.Price, V.Sid
         FROM Games G INNER JOIN VideoGamesShops V ON G.Gid = V.Bid) VG
...
SELECT S.Sid, MIN(VG.NoOfEmployees) MinNoOfEmployees
FROM Shops S
INNER JOIN VideoGamesCompany VG ON S.Sid = VG.Vid
GROUP BY S.Sid
HAVING MIN(VG.NoOfEmployees) > 50;
...
GROUP BY S.Sid
HAVING AVG(VGS.StockQuantity) > (SELECT AVG(StockQuantity) FROM VideoGamesShops);


SELECT S.City, sum(VGS.StockQuantity) AvgStock
FROM Shops S
INNER JOIN VideoGamesShops VGS ON S.Sid = VGS.Sid
GROUP BY S.City, S.Sid
